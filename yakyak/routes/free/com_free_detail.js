...
router.get('/', function(req, res, next){
    let { num } = req.query;
    n=req.query;
    var sql = 'SELECT * FROM free WHERE free_no=\''+num+';\'';
    var sqls = 'SELECT * FROM free_comment where free_no=\''+num+';\''; 
    var sqlss = 'UPDATE free set free_view=? WHERE free_no=\''+num+'\'';
    var view;
    conn.query(sql, function (err, rows, fields) {
        if(err) console.log('query is not excuted. select fail...\n' + err);
        else {
            view=parseInt(rows[0].free_view)+1;
            console.log(view);
                    ...
                    conn.query(sqlss, view, function(err) {
                        if(err) console.log('query is not excuted. select fail...\n' + err);
                            ...
                            if(req && req.session && req.session.count) { 
                                // 세션이 존재하는 경우
                                res.render('com_free_detail.ejs',{user:req.session.uid, num:num,list : rows, comment:rows2});
                                console.log(rows);
                        
                            } else { 
                                // 세션이 존재하지 않는 경우
                                var id='';
                                res.render('com_free_detail.ejs', {user:id, list : rows, comment:rows2});
                            }
                        }
                    });
                }
    ...
    var sql2 = 'SELECT COUNT(*) AS count FROM free_comment';
    console.log(sql2);
...
router.post('/', function (req, res) {
    ...
    var sql = 'INSERT INTO free_comment VALUES(?, ?, ?, ?, NOW())';
    var sqls = 'UPDATE free set free_comment=? WHERE free_no=?';
