    ...
    conn.query(sql, function (err, rows, fields) {
        if(err) console.log('query is not excuted. select fail...\n' + err);
            ...
            if(req && req.session && req.session.count) { 
                // 세션이 존재하는 경우
                res.render('com_free_update.ejs',{user:req.session.uid, num:num,list : rows});
                console.log(rows);
        
            } else { 
                // 세션이 존재하지 않는 경우
                var id='';
                res.render('com_free_update.ejs', {user:id, list : rows});
            }
        }
    });
});
...
router.post('/', function (req, res) {
    ...
    var sqls = 'UPDATE free set free_title=?, free_content=? WHERE free_no=?';
