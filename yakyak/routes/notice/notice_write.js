...
router.get('/', function (req, res) {
    var sql = 'SELECT COUNT(*) AS count FROM notice';
    console.log(sql);
    ...
    if(req && req.session && req.session.count) { 
        // 세션이 존재하는 경우
        res.render('notice_write.ejs',{user:req.session.uid});

    } else { 
        // 세션이 존재하지 않는 경우
        var id='';
        res.render('notice_write.ejs', {user:id});
    }
});
...
router.post('/', function (req, res) {
    ...
    var sql = 'INSERT INTO notice VALUES(?, ?, ?, ?, ?, NOW(), ?)';
