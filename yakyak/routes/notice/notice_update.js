    ...
    conn.query(sql, function (err, rows, fields) {
        if(err) console.log('query is not excuted. select fail...\n' + err);
            ...
            if(req && req.session && req.session.count) { 
                // 세션이 존재하는 경우
                res.render('notice_update.ejs',{user:req.session.uid, num:num,list : rows});
                console.log(rows);
        
            } else { 
                // 세션이 존재하지 않는 경우
                var id='';
                res.render('notice_update.ejs', {user:id, list : rows});
            }
        }
    });
});
...
router.post('/', function (req, res) {
    ...
    var sqls = 'UPDATE notice set notice_title=?, notice_content=? WHERE notice_no=?';
    ...
    conn.query(sqls, paramss, function(err) {
        if(err) console.log('query is not excuted. insert fail...\n' + err);
            ...
            console.log("글 수정 완료");
            res.redirect('/notice_detail?num='+n.num);
        }
    });
});
