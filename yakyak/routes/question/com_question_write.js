...
router.get('/', function (req, res) {
    var sql = 'SELECT COUNT(*) AS count FROM quest';
    console.log(sql);
    ...
    if(req && req.session && req.session.count) { 
        // 세션이 존재하는 경우
        res.render('com_question_write.ejs',{user:req.session.uid});

    } else { 
        // 세션이 존재하지 않는 경우
        var id='';
        res.send('<script>alert("로그인이 필요한 서비스입니다.");location.href="/question?page=1";</script>');
        // res.render('com_question_write.ejs', {user:id});
    }
});
router.post('/', function (req, res) {
    ...
    var sql = 'INSERT INTO quest VALUES(?, ?, ?, ?, ?, NOW(), ?)';
